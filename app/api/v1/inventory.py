...
async def list_products(
    ...
    items, total = await inventory_service.list_products(
        db, org_id, category=category, low_stock_only=low_stock_only,
...
async def create_product(data: ProductCreate, db: DbSession, current: CurrentUserDep, org_id: ResolvedOrgDep) -> ProductResponse:
    current.require_permission("inventory.write")
    product = await inventory_service.create_product(db, org_id, data)
    return ProductResponse.model_validate({**product.__dict__, "current_stock": 0})


@router.patch("/products/{product_id}", response_model=ProductResponse)
async def update_product(product_id: UUID, data: ProductUpdate, db: DbSession, current: CurrentUserDep, org_id: ResolvedOrgDep) -> ProductResponse:
    current.require_permission("inventory.write")
    product = await inventory_service.update_product(db, org_id, product_id, data)
...
async def delete_product(product_id: UUID, db: DbSession, current: CurrentUserDep, org_id: ResolvedOrgDep) -> None:
    current.require_permission("inventory.write")
    await inventory_service.delete_product(db, org_id, product_id)
...
async def list_stock_entries(
    ...
    current.require_permission("inventory.read")
    items, _ = await inventory_service.list_stock_entries(db, org_id, product_id, page=page, page_size=page_size)
...
async def add_stock_entry(data: StockEntryCreate, db: DbSession, current: CurrentUserDep, org_id: ResolvedOrgDep) -> StockEntryResponse:
    current.require_permission("inventory.write")
    entry = await inventory_service.add_stock_entry(db, org_id, data, current.user.id)
